...
graph TD
    ...
    Col1 --> D1["Doc: {id:1, name:'Boris'}"]
    Col1 --> D2["Doc: {id:2, name:'Ilyas'}"]
...
| Task | SQL | MongoDB |
| :--- | :--- | :--- |
| **Create Record** | `INSERT INTO users (name, age) VALUES ('Boris', 20)` | `db.users.insertOne({name: 'Boris', age: 20})` |
| **Select All** | `SELECT * FROM users` | `db.users.find()` |
| **Filter by Condition** | `SELECT * FROM users WHERE age > 18` | `db.users.find({age: {$gt: 18}})` |
| **Update Record** | `UPDATE users SET age = 21 WHERE name = 'Boris'` | `db.users.updateOne({name: 'Boris'}, {$set: {age: 21}})` |
| **Delete Record** | `DELETE FROM users WHERE age < 10` | `db.users.deleteMany({age: {$lt: 10}})` |

<!-- QUIZ_START 
[
    {
        "question": "In what format does MongoDB physically store data?",
        "options": ["JSON", "XML", "BSON", "CSV"],
        "correctIndex": 2
    },
    {
        "question": "Which process in a MongoDB cluster is responsible for routing requests to the correct shards?",
        "options": ["mongod", "mongos", "Config Server", "Replica Set"],
        "correctIndex": 1
    },
    {
        "question": "What happens if the Primary node in a Replica Set fails?",
        "options": ["The database permanently stops accepting writes", "Secondary nodes hold an election for a new Primary", "The application must manually switch to a Secondary", "Data is automatically converted to SQL"],
        "correctIndex": 1
    }
]
QUIZ_END -->
