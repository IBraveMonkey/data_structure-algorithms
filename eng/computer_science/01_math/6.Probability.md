...
func main() {
    ...
    fmt.Printf("Estimated P(Two Heads) ≈ %.3f (Expected 0.250)\n", probability)
}
```
...
func main() {
    ...
    fmt.Printf("Estimated P(Two Aces) ≈ %.4f (Expected 0.0045)\n", probability)
}
```
...
Since you pay \$100 to "buy" an expected value of **\$11**, this is mathematically a losing proposition! 📉
...
**Idea**: Randomly throw "darts" into a 1×1 square. The ratio of points that land inside a quarter-circle to the total number of points approximates $\pi/4$.
...
func estimatePi(trials int) float64 {
    ...
    for i := 0; i < trials; i++ {
        // Random (x, y) coordinates within the square [0, 1] × [0, 1]
        x := rand.Float64()
        y := rand.Float64()
        
        // Check if the point is within the circle unit border: x² + y² ≤ 1
        if x*x+y*y <= 1.0 {
            insideCircle++
        }
    }
    
    // π ≈ 4 * (ratio of points inside the circle)
...
func main() {
    ...
    fmt.Printf("Estimated π ≈ %.5f (Actual π ≈ 3.14159)\n", pi)
}
```
...
func main() {
    ...
    // Theoretical: 8 combinations out of 36 give 7 or 11 (8/36 ≈ 0.222)
    fmt.Printf("Estimated P(Win) ≈ %.3f\n", probability)
}
```
...
[
    {
        "question": "What is the probability that in two consecutive flips of a fair coin, 'Heads' will appear both times?",
        "options": ["0.5", "0.25", "0.75", "1.0"],
        "correctIndex": 1
    },
    {
        "question": "What is the 'Expected Value' (EV) of a random variable?",
        "options": ["The most likely result", "The maximum possible result", "The long-term average result over many repetitions", "The first result obtained"],
        "correctIndex": 2
    },
    {
        "question": "What is the core idea behind the Monte Carlo method?",
        "options": ["Strict mathematical proof", "Using random numbers and multiple simulations to solve problems", "Sieving out prime numbers", "Sorting data by probability"],
        "correctIndex": 1
    }
]
QUIZ_END -->
