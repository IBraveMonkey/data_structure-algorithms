...
> - In JSON, every key is repeated for every object: `{"name": "Boris", "age": 30}` → 26 bytes.
> - In Protobuf, keys are replaced by numeric tags: `\x0A\x04Boris\x10\x1E` → 8 bytes.
...
[
    {
        "question": "What key advantage of Protocol Buffers over JSON provides higher performance in gRPC?",
        "options": ["Human-readability of data", "Binary format and no key duplication (using tags)", "Ability to work without a schema", "Support only in the Go language"],
        "correctIndex": 1
    },
    {
        "question": "On which version of the HTTP protocol is the gRPC transport layer based?",
        "options": ["HTTP/1.1", "HTTP/2", "HTTP/3 (QUIC)", "WebSocket"],
        "correctIndex": 1
    },
    {
        "question": "Which type of RPC call in gRPC allows the client and server to exchange streams of messages simultaneously?",
