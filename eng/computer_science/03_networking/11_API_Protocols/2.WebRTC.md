...
## 🗺️ The Connection Process (Signaling)
...
[
    {
        "question": "What is the primary architectural feature of WebRTC regarding media data transmission (audio/video)?",
        "options": ["Data always passes through a central server", "Data is transmitted directly between browsers (Peer-to-Peer)", "Data is transmitted only via TCP", "WebRTC requires mandatory third-party plugins"],
        "correctIndex": 1
    },
    {
        "question": "Why is a Signaling server needed in WebRTC if media is transmitted directly P2P?",
        "options": ["To encrypt the video stream", "To exchange metadata (SDP) and network candidates to establish the connection", "To store call recordings", "A signaling server is not required"],
        "correctIndex": 1
    },
    {
        "question": "What role does a TURN server play in the WebRTC infrastructure?",
        "options": ["Compresses video to save traffic", "Relays media data when a direct P2P connection is impossible due to network restrictions (NAT/Firewall)", "Generates unique IDs for users", "Checks if the user has a camera"],
        "correctIndex": 1
    }
]
QUIZ_END -->
