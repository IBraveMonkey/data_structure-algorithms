...
A server that sits **in front of your services**. It protects the servers from the public internet.
...
### Why is it used?
- **SSL Termination**: The proxy handles the computationally expensive HTTPS decryption/encryption, passing standard HTTP to the service.
...
[
    {
        "question": "What primary role does a Reverse Proxy, such as Nginx, play in a web application architecture?",
        "options": ["Hides the user's IP address from the internet", "Protects application servers by performing SSL termination and routing", "Speeds up server RAM", "Provides Wi-Fi access"],
        "correctIndex": 1
    },
    {
        "question": "What is the name of the load balancing algorithm where requests are distributed strictly in sequence among all available servers?",
        "options": ["Least Connections", "IP Hash", "Round Robin", "Random Selection"],
        "correctIndex": 2
    },
    {
        "question": "What additional function, critical for microservices, does an API Gateway typically take on?",
        "options": ["Cooling the server rack", "JWT token validation and rate limiting", "Replacing hard drives", "Go garbage collection"],
        "correctIndex": 1
    }
]
QUIZ_END -->
