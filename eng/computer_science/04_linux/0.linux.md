...
[
    {
        "question": "What does the term 'everything is a file' mean in Linux?",
        "options": ["All programs are written in the same language", "Devices, processes, and sockets are represented as files in the system", "Linux has no directories, only files", "Every file must have a .txt extension"],
        "correctIndex": 1
    },
    {
        "question": "Which Linux directory is intended for storing system configuration files?",
        "options": ["/bin", "/home", "/etc", "/var"],
        "correctIndex": 2
    },
    {
        "question": "What does the `|` (pipe) operator do in the shell?",
        "options": ["Copies a file", "Deletes a process", "Passes the output of one command as input to another", "Compares two text files"],
        "correctIndex": 2
    }
]
QUIZ_END -->