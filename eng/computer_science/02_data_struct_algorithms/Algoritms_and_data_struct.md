...
**Description**: 
- **Classical Array (Fixed-size)** — is a data structure representing a sequence of elements of the same type stored in a contiguous memory area. The size of the array is fixed at creation and cannot be changed later.
- **Dynamic Array** — is an evolution of the classical array concept that can change its size during program execution. It automatically increases or decreases as needed.

Elements are accessed by index with O(1) complexity in both cases.
...
func main() {
    // Classical array (fixed size)
    var classicArr [4]int = [4]int{1, 2, 3, 4}
    fmt.Println("Classical array:", classicArr)
    
    // Access O(1) to element
    fmt.Println(classicArr[2]) // Output: 3
    ...
    fmt.Println("After adding element:", dynamicArr) // [1 2 3 4 5]

    // Insertion in the middle O(n)
    index := 2
    dynamicArr = append(dynamicArr[:index], append([]int{10}, dynamicArr[index:]...)...)
    fmt.Println("After inserting in the middle:", dynamicArr) // [1 2 10 3 4 5]
}
```
