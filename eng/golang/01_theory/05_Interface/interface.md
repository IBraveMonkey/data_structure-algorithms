...
func main() {
    en := EnglishGreeter{name: "Boris"}
    ru := RussianGreeter{name: "Boris"}
    
    SayHello(en) // Hello, Boris
    SayHello(ru) // Привет, Boris
}
```
...
[
    {
        "question": "Do you need to use the 'implements' keyword to make a type implement an interface in Go?",
        "options": [
            "Yes, always",
            "Only for exported interfaces",
            "No, implementation is implicit if the type has all the required method signatures",
            "Only if the struct has pointer receivers"
        ],
        "correctIndex": 2
    },
    {
        "question": "What two internal fields does an interface variable store?",
        "options": [
            "Value and Address",
            "Concrete Type and Concrete Value",
            "Name and Method List",
            "Length and Capacity"
        ],
        "correctIndex": 1
    },
    {
        "question": "When is an interface variable equal to 'nil' in Go?",
        "options": [
            "When the concrete value is nil",
            "When the concrete type is nil",
            "Only when BOTH the concrete type and concrete value are nil",
...
func main() {
    ...
    emailService.RegisterUser("Boris")
    ...
    testService.RegisterUser("Ilyas")
