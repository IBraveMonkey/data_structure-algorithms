...
[
    {
        "question": "Why is alignment important for sync.WaitGroup on 32-bit architectures?",
        "options": [
            "To save memory",
            "To prevent a crash during atomic operations on the state field, which requires 64-bit alignment",
    ...
    {
        "question": "What is 'Starvation Mode' in a sync.Mutex?",
        "options": [
            "When the processor runs out of power",
            "A mode triggered if a goroutine waits for more than 1ms, ensuring it gets the lock next without competing with new incoming goroutines",
            "Decreasing the memory usage",
            "Deleting unused goroutines"
        ],
        "correctIndex": 1
    },
    {
        "question": "What is 'Writer Priority' in sync.RWMutex?",
        "options": [
            "Writers are paid more",
            "Incoming readers are blocked if a writer is already waiting, preventing the writer from being starved by a constant stream of readers",
            "Writers can bypass all locks",
            "It doesn't exist"
        ],
        "correctIndex": 1
    }
