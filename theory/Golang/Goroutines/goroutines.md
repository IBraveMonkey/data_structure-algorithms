...
### Анатомия переключения контекста (Context Switching)

Чтобы понять, почему горутины такие быстрые, нужно сравнить их переключение с переключением потоков ОС.

#### 1. Потоки ОС (Kernel-level Switch)
Когда ОС переключает один поток на другой, происходит следующее:
- **Переход в режим ядра**: Процессор переключается из User Mode в Kernel Mode (это дорогой системный вызов).
- **Сохранение регистров**: ОС должна сохранить значения **всех** регистров процессора (их десятки: AVX, SSE, Floating Point и т.д.).
- **Scheduler ОС**: Ядро решает, какой поток запустить следующим.
- **Очистка кэшей (TLB Flush)**: Если новый поток принадлежит другому процессу, ОС должна сбросить таблицу страниц (TLB), что делает доступ к памяти медленным на первое время ("холодный старт").
- **Размер контекста**: Обычно переключение потока ОС занимает **~1-10 микросекунд** и сохраняет несколько килобайт данных.

#### 2. Горутины (User-space Switch)
Планировщик Go работает полностью в пользовательском пространстве. Переключение происходит так:
- **Только регистры управления**: Сохраняются всего 3 ключевых регистра: **PC** (Program Counter), **SP** (Stack Pointer) и **DX**.
- **User-space**: Не нужно входить в режим ядра. Это просто вызов функции внутри runtime.
- **Cache Locality**: Поскольку горутины одного процесса делят одну и ту же память, кэши CPU (L1/L2/L3) не нужно сбрасывать. Данные, с которыми работала первая горутина, могут быть полезны второй.
- **Размер контекста**: Переключение горутины занимает **~10-50 наносекунд**.

> [!TIP]
> **Промывка кэша (Cache Pollution)**: В ОС при смене потока новый поток приносит свои данные, "вымывая" данные старого потока из кэша процессора. В Go планировщик старается запускать горутины на тех же ядрах, где они работали раньше (Affinity), чтобы данные оставались в кэше L1/L2.
