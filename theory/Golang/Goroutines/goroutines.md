...
> [!IMPORTANT]
> **M:N Модель**: Горутины — это не потоки ОС. Это лёгкие абстракции (User-space threads). Пакет runtime Go отображает **M** горутин на **N** системных потоков, что позволяет эффективно управлять тысячами задач на малом количестве ресурсов.
