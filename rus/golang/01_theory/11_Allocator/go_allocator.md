# ⚙️ Аллокатор памяти в Go  


## 🏗️ Как Go управляет кучей: Схемы, структуры и оптимизации
...
> [!NOTE]
> Такая архитектура делает Go одним из самых быстрых языков в плане работы с памятью среди языков с Garbage Collector(не точно).

<!-- QUIZ_START 

[
    {
        "question": "На какой известной архитектуре основан аллокатор памяти Go?",
        "options": [
            "DLMalloc",
    ...
    {
        "question": "Что такое 'mspan' в контексте управления памятью Go?",
        "options": [
            "Имя процесса Go в системе",
            "Базовый блок памяти, состоящий из одной или нескольких страниц и нарезанный на объекты одного размера",
            "Указатель на Garbage Collector",
            "Тип данных для хранения строк"
        ],
        "correctIndex": 1
    },
    {
        "question": "Для чего каждому процессору P назначен свой локальный кэш 'mcache'?",
        "options": [
            "Чтобы хранить там код программы",
            "Чтобы выделять память под маленькие объекты без использования блокировок (lock-free), что ускоряет работу",
