...
[
    {
        "question": "Что такое AST (Abstract Syntax Tree) в процессе компиляции Go?",
        "options": [
            "Список всех переменных программы",
            "Древовидное представление структуры исходного кода после синтаксического анализа",
            "Промежуточный код для процессора",
            "Таблица всех импортируемых пакетов"
        ],
        "correctIndex": 1
    },
    {
        "question": "На каком этапе компиляции Go выполняется оптимизация 'Escape Analysis' (анализ побега)?",
        "options": [
            "Frontend (во время парсинга)",
            "Middle-end (на этапе SSA)",
            "Backend (при генерации ассемблера)",
            "Во время линковки"
        ],
        "correctIndex": 1
    },
    {
        "question": "Для чего Go использует SSA (Static Single Assignment) в качестве промежуточного представления?",
        "options": [
            "Для упрощения синтаксиса языка",
            "Для глубоких оптимизаций, так как каждой переменной значение присваивается ровно один раз",
            "Для обратной совместимости с C++",
            "Для ускорения работы линковщика"
        ],
        "correctIndex": 1
    }
]
