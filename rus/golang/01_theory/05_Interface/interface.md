...
[
    {
        "question": "Как в Go реализовано соответствие типа интерфейсу?",
        "options": [
            "Через ключевое слово implements",
            "Явно в определении структуры",
            "Неявно: если тип реализует все методы интерфейса, он считается реализующим этот интерфейс",
            "Через регистрацию типа в специальном реестре runtime"
        ],
        "correctIndex": 2
    },
    {
        "question": "Из каких двух полей состоит переменная типа интерфейс на низком уровне?",
        "options": [
            "Указатель на данные и размер данных",
            "Указатель на таблицу методов (itab/type info) и указатель на конкретные данные (value)",
            "Имя интерфейса и количество методов",
            "Только указатель на данные"
        ],
        "correctIndex": 1
    },
    {
        "question": "При каком условии выражение `w == nil` для переменной интерфейса `var w io.Writer` вернет `true`?",
        "options": [
            "Если конкретное значение (value) равно nil",
            "Если тип (type) равен nil",
            "Только когда и поле типа, и поле значения одновременно равны nil",
            "Если интерфейс пустой (any)"
        ],
        "correctIndex": 2
    }
]
...
graph TB
    ...
    style A fill:#009688,color:#fff,stroke:#333
...
graph TD
    ...
    style E fill:#009688,color:#fff,stroke:#333
...
func main() {
    en := EnglishGreeter{name: "Борис"}
    ru := RussianGreeter{name: "Борис"}
    
    SayHello(en) // Hello, Борис
    SayHello(ru) // Привет, Борис
}
```
...
func main() {
    ...
    emailService.RegisterUser("Борис")
    ...
    testService.RegisterUser("Ильяс")
