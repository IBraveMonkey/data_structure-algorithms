...
### ⚙️ Как каналы работают под капотом?
...
## 🅿️ Парковка горутин (Gopark и Goready)
...
graph TD
    ...
    P --> Wait[G1 в waiting, добавлена in sendq]
...
[
    {
        "question": "Как ведут себя небуферизированные каналы в Go?",
        "options": [
            "Они всегда асинхронны",
            "Они блокируют отправителя до тех пор, пока получатель не прочитает данные (и наоборот)",
            "Они хранят до 10 элементов",
            "Они никогда не блокируются"
        ],
        "correctIndex": 1
    },
    {
        "question": "Какая оптимизация используется при передаче данных через небуферизированный канал, когда и отправитель, и получатель готовы?",
        "options": [
            "Данные записываются в базу данных",
            "Direct Copy: данные копируются напрямую из стека одной горутины в стек другой",
            "Данные сжимаются",
            "Используется глобальный кэш"
        ],
        "correctIndex": 1
    },
    {
        "question": "Что произойдет при попытке записи в закрытый канал?",
        "options": [
            "Программа просто проигнорирует запись",
            "Запись вернет ошибку",
            "Произойдет panic",
