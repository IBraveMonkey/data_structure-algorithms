...
```
**Память:** `[b][b][b][b][b][b][b][b] [a][p][p][p][p][p][p][p]`

> [!TIP]
> **Почему оба примера по 16 байт?**
> Итоговый размер структуры всегда должен быть кратен её максимальному выравниванию (в данном случае **8** для `int64`).
> В обоих случаях `8 + 1 = 9`, а ближайшее число, кратное 8 — это **16**.
>
> **Почему `Good` лучше?**
> В `Good` пустые байты (padding) собраны в конце. Если добавить еще одно поле `c bool`, оно займет место существующего padding:
> - `Good` + `c bool` = **16 байт**
> - `Bad` + `c bool` (в конце) = **24 байт**
...
[
    {
        "question": "Что такое выравнивание (alignment) памяти в Go?",
        "options": [
            "Автоматическая сортировка полей структуры по алфавиту",
            "Правило, согласно которому адрес значения в памяти должен быть кратен определённому числу",
            "Сжатие данных для экономии места",
            "Процесс удаления неиспользуемых переменных"
        ],
        "correctIndex": 1
    },
    {
        "question": "Почему выравнивание данных важно для производительности процессора?",
        "options": [
            "Процессор читает память байт за байтом",
            "Выровненные данные позволяют процессору прочитать значение за один цикл (одно чтение памяти)",
            "Это экономит заряд батареи",
            "Это никак не влияет на скорость"
        ],
        "correctIndex": 1
    },
    {
        "question": "Какое 'правило большого пальца' помогает минимизировать padding в структурах?",
        "options": [
            "Располагать поля в алфавитном порядке",
            "Располагать поля в порядке убывания их размера (от больших к меньшим)",
            "Использовать только типы одного размера",
            "Располагать поля в случайном порядке"
        ],
        "correctIndex": 1
    }
]
