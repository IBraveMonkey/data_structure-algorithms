...
> [!WARNING]
> **Важный нюанс**: Escape Analysis — это набор эвристик, которые меняются от версии к версии Go. То, что "улетало" в кучу в Go 1.10, может остаться на стеке в Go 1.24. Невозможно со 100% уверенностью предсказать поведение компилятора без использования флагов диагностики (`-gcflags="-m"`). Даже если код кажется очевидным, сложные оптимизации (например, inlining) могут внезапно изменить результат.
...
## ⚙️ Как работает Escape Analysis
...
func createUser() *User {
    u := User{Name: "Борис"}  // u escapes to heap
    return &u
}
```
...
func addUser() {
    u := User{Name: "Ильяс"}  // u escapes to heap
...
[
    {
        "question": "Что такое Escape Analysis в Go?",
        "options": [
            "Процесс поиска утечек памяти во время выполнения",
            "Анализ компилятора для решения, где выделить память: на стеке или в куче",
            "Механизм защиты от несанкционированного доступа к памяти",
            "Алгоритм сжатия бинарного файла"
        ],
        "correctIndex": 1
    },
    {
        "question": "В чем главное преимущество выделения памяти на стеке (stack) по сравнению с кучей (heap)?",
        "options": [
            "На стеке можно хранить больше данных",
            "Выделение на стеке происходит мгновенно и не нагружает Garbage Collector",
            "Стековая память доступна всем горутинам",
            "Стек автоматически шифруется"
        ],
        "correctIndex": 1
    },
    {
        "question": "В каком случае переменная почти наверняка 'убежит' в кучу (heap)?",
        "options": [
            "Если она используется только как локальный счетчик в цикле",
            "Если функция возвращает указатель на локальную переменную",
            "Если переменная передается в другую локальную функцию по значению",
            "Если это переменная типа bool"
        ],
        "correctIndex": 1
    }
]
