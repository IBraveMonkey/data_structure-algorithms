# Defer в Go: Полное руководство ⏱️
...
### Важные ограничения Panic/Recover: ⚠️
...
[
    {
        "question": "В каком порядке выполняются функции, отложенные через ключевое слово `defer`?",
        "options": [
            "В порядке их объявления (FIFO)",
            "В обратном порядке объявления (LIFO)",
            "Случайным образом",
            "Одновременно в разных горутинах"
        ],
        "correctIndex": 1
    },
    {
        "question": "Когда вычисляются аргументы функции, передаваемой в `defer`?",
        "options": [
            "В момент фактического выполнения функции",
            "В момент выхода из родительской функции",
            "В момент объявления (вызова слова defer)",
            "Никогда не вычисляются"
        ],
        "correctIndex": 2
    },
    {
        "question": "Может ли `defer` изменить значение, возвращаемое функцией?",
        "options": [
            "Нет, это невозможно",
            "Да, если возвращаемое значение именованное (например, func foo() (x int))",
            "Только если функция возвращает ошибку (error)",
            "Только в main горутине"
        ],
        "correctIndex": 1
    }
]
